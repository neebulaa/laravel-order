...
<form class="w-100 mt-4" style="max-width: 400px" action="/items/{{ $item->id }}" method="POST">
    @method('PUT')
    @csrf
    ...
    <div class="mb-3">
        ...
        <select name="type" id="type" class="form-select @error('type') is-invalid @enderror">
            <option value="" {{ old('type', $item->type) == '' ? 'selected' : '' }}>Choose type</option>
    ...
    <div class="mb-3">
        ...
        <div class="input-group">
            ...
            <input type="number" min="0" class="form-control @error('stock') is-invalid @enderror" name="stock" min="1" value="{{ old('stock', $item->stock) }}">
        </div>
        @error('stock')
            <p class="text-danger mt-1">{{ $message }}</p>
        @enderror
    ...
    <div class="mb-3">
        ...
        <div class="input-group">
            ...
            <input type="number" class="form-control @error('price') is-invalid @enderror" name="price" min="0" step="any" value="{{ old('price', $item->price) }}">
        </div>
        @error('price')
            <p class="text-danger mt-1">{{ $message }}</p>
        @enderror
